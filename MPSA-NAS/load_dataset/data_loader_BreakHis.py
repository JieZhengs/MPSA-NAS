...
def get_train_valid_loader(data_dir_train,
    ...
    # print("train_dataset" + str(train_dataset.classes))

    valid_dataset = datasets.ImageFolder(data_dir_train, transform=valid_transform)
    # print("valid_dataset"+ str(valid_dataset.classes))
...
# 7100张训练集
def get_train_loader(data_dir_train,
                     batch_size,
                     augment,
                     shuffle=True,
                     num_workers=4,
    ...
    if augment:
        ...
        train_transform.transforms.append(Cutout(64))
    else:
        train_transform = transforms.Compose([
            transforms.ToTensor()
        ])
    ...
    # load the dataset
    # train_dataset = MyData(data_dir_train,transform=train_transform)
    train_dataset = datasets.ImageFolder(data_dir_train, transform=train_transform)
...
# 800多张
def get_test_loader(data_test_dir,
                    batch_size,
                    shuffle=False,
                    num_workers=4,
                    pin_memory=False):
    ...
    dataset = datasets.ImageFolder(data_test_dir, transform=transform)
