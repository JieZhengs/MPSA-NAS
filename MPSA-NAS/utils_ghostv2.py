...
class Utils(object):
    ...
    @classmethod
    def save_population_and_err(cls, type, population, err_set, num_parameters, flops, gen_no, proxy_err=None,
                                time=None):
    ...
    def popAndErr2str(cls, population, err_set, num_parameters, flops, proxy_err, time):
        ...
        for id, particle in enumerate(population):
            ...
            for j, subParticle in enumerate(subParticles):
                ...
                if j > 0:
                    _str.append('Transition layer, stride=2, %d*%d Ghosnetv2, in:%d, out:%d' % (
    ...
    def calc_in_out_channels(cls, valid_particle, block_idx=None, end_channel=0):
        ...
        if block_idx == 0:
            if dataset == 'cifar10' or dataset == 'cifar100':
                node1_filter = 16
            elif dataset == 'imagenet' or dataset == 'LC25000' or dataset == 'BreakHis' or dataset == 'ICIAR' or dataset == 'PCAM' or dataset == 'Lymphoma' or dataset == 'Colorectal' or dataset == 'road':
                node1_filter = 32
        else:
    ...
    def read_template(cls, test_model=False):
        ...
        if test_model:
            ...
            _path = './template/' + dataset + '_ghost.py'
        part1 = []
        part2 = []
        part3 = []
    ...
    def generate_pytorch_file(cls, particle, curr_gen, id, test_model=False):
        ...
        for j, sub_particle in enumerate(subParticles):
            ...
            if j > 0:
                ...
                strided_conv = '%s = GhostBottleneckV2(in_chs=%d, out_chs=%d, expantion=3, dw_kernel_size=%d, stride=2, se_ratio=0.25)' % (
                    stride_convname, int(end_channel), outC_strided, kernel_sizes[j + 1],
                    )
